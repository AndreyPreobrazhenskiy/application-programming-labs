...
class ImageViewer(QMainWindow):
    ...
    def setup_ui(self) -> None:
        ...
        self.image_label.setStyleSheet(
            "background-color: #007fff; border: 2px solid #adaaaa; border-radius: 5px;"
        )
        layout.addWidget(self.image_label)

        self.load_dataset_button = QPushButton("Load Dataset")
        self.load_dataset_button.setFixedSize(1000, 50)
        self.load_dataset_button.setStyleSheet(
            "border: 2px solid #ababab; border-radius: 5px; background-color: #6495ed;"
        )
    ...
    def load_dataset(self) -> None:
        ...
        if file_path:
            try:
                self.dataset_iterator = iter(Iterator(file_path))
